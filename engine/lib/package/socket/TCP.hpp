...
namespace Socket
{
    ...
    Ip TCP::ip(void) const
    {
        return this->_address.ip();
    }

    Port TCP::port(void) const
    {
    ...
    TCP TCP::accept(void)
    {
        ...
        ret.close();
        ret._socket_id = ::accept(this->_socket_id, (struct sockaddr*)&addr, &len);
        ret._opened = true;
        ret._binded = true;
        ret._address = Address(addr);
    ...
    int TCP::send(const T* buffer, size_t len)
    {
        ...
        int ret;
        if ((ret = ::send(this->_socket_id, (const char*)buffer, len, MSG_WAITALL)) == -1) throw SocketException("[send] Cannot send");
        return ret;
    }
    ...
    int TCP::recv(T* buffer, size_t len)
    {
        ...
        {
            stringstream error;
            error << "[recv] [len=" << len << "] Data length higher then max buffer len (" << SOCKET_MAX_BUFFER_LEN << ")";
            throw SocketException(error.str());
        }

        int ret;
        if ((ret = ::recv(this->_socket_id, (char*)buffer, len, MSG_WAITALL)) == -1) throw SocketException("[send] Cannot recv");
        return ret;
    }

    void TCP::send_file(const std::string& file_name)
    {
    ...
    void TCP::recv_file(const std::string& file_name)
    {
        unsigned long long file_size;
        char chunk[SOCKET_MAX_BUFFER_LEN];
        char sync;
