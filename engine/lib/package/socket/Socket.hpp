...
namespace Socket
{
    ...
    public:
        ...
        template <class T> int recv(Address*, T*, size_t, unsigned int*);
        template <class T> Datagram<T*> recv(T*, size_t = SOCKET_MAX_BUFFER_LEN);
        template <class T, size_t N> Datagram<T[N]> recv(size_t = N);
        template <class T> Datagram<T> recv(void);
        template <class T> Datagram<vector<T> > recv(size_t);
    };
    ...
    public:
        ...
        void recv_file(const std::string&);
    };
}
