...
namespace Socket
{
    ...
    int UDP::send(Ip ip, Port port, const T *data, size_t len)
    {
        ...
        if ((ret = sendto(this->_socket_id, (const char*)data, len, MSG_WAITALL, (struct sockaddr*)&address, sizeof(struct sockaddr))) == -1)
        {
            stringstream error;
    ...
    template <class T>
    int UDP::recv(Address *address, T *data, size_t len, unsigned int *received_elements)
    {
        if (!this->_opened) this->open();
        if (!this->_binded) throw SocketException("[recv] Make the socket listening before receiving");
        ...
        if ((received_bytes = recvfrom(this->_socket_id, (char*)data, len, MSG_WAITALL, (struct sockaddr*)address, (socklen_t*)&size)) == -1)
        {
            throw SocketException("[recv] Cannot recv");
        }
    ...
    template <class T>
    Datagram<T*> UDP::recv(T *buffer, size_t len)
    {
        Datagram<T*> ret;
        ret.received_bytes = this->recv<T>(&ret.address, buffer, len, &ret.received_elements);
        ret.data = buffer;
        return ret;
    }

    template <class T, size_t N>
    Datagram<T[N]> UDP::recv(size_t len)
    {
        Datagram<T[N]> ret;
        ret.received_bytes = this->recv<T>(&ret.address, ret.data, len, &ret.received_elements);
        return ret;
    }

    template <class T>
    Datagram<T> UDP::recv(void)
    {
        Datagram<T> ret;
        ret.received_bytes = this->recv<T>(&ret.address, &ret.data, 1, &ret.received_elements);
        return ret;
    }

    template <>
    Datagram<string> UDP::recv<string>(void)
    {
        Datagram<string> ret;
        char buffer[SOCKET_MAX_BUFFER_LEN];

        ret.received_bytes = this->recv<char>(&ret.address, buffer, SOCKET_MAX_BUFFER_LEN, &ret.received_elements);
        ret.data = buffer;
    ...
    template <class T>
    Datagram<vector<T> > UDP::recv(size_t len)
    {
